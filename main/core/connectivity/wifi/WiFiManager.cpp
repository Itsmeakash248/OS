...
esp_err_t WiFiManager::init(lv_subject_t *connected_subject,
                            lv_subject_t *ssid_subject,
                            lv_subject_t *ip_subject,
...
esp_err_t WiFiManager::connect(const char *ssid, const char *password) {
  if (!m_is_enabled) return ESP_ERR_INVALID_STATE;
...
esp_err_t WiFiManager::setEnabled(bool enabled) {
  if (m_is_enabled == enabled) return ESP_OK;
  ...
  if (!enabled) {
    ...
    return ConnectivityManager::getInstance().setWifiMode(ap_enabled ? WIFI_MODE_AP : WIFI_MODE_NULL);
  } else {
    setStatus(WiFiStatus::DISCONNECTED);
    return ConnectivityManager::getInstance().setWifiMode(ap_enabled ? WIFI_MODE_APSTA : WIFI_MODE_STA);
  }
}

esp_err_t WiFiManager::scan(ScanCallback callback) {
  if (!m_is_enabled) return ESP_ERR_INVALID_STATE;
  if (m_is_scanning) return ESP_ERR_INVALID_STATE;
...
void WiFiManager::wifi_event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_id == WIFI_EVENT_STA_START) {
    ...
    bool is_auth_failure = (event->reason == WIFI_REASON_AUTH_EXPIRE || 
    ...
    self->setStatus(self->isConnected() ? WiFiStatus::CONNECTED : 
                   (self->m_is_enabled ? WiFiStatus::DISCONNECTED : WiFiStatus::DISABLED));
  }
}
