...
esp_err_t SystemManager::initGuiState() {
  ...
  add_obs(&m_brightness_subject, [](lv_observer_t *, lv_subject_t *s) {
    if (auto d = lv_display_get_default()) lv_lovyan_gfx_set_brightness(d, (uint8_t)lv_subject_get_int(s));
  });
  ...
  add_obs(&m_rotation_subject, [](lv_observer_t *, lv_subject_t *s) {
    if (auto d = lv_display_get_default()) lv_display_set_rotation(d, (lv_display_rotation_t)(lv_subject_get_int(s) / 90));
  });
  ...
  if (auto d = lv_display_get_default()) {
    lv_lovyan_gfx_set_brightness(d, (uint8_t)lv_subject_get_int(&m_brightness_subject));
    lv_display_set_rotation(d, (lv_display_rotation_t)(lv_subject_get_int(&m_rotation_subject) / 90));
  }
  GuiTask::unlock();
