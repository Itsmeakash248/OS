...
public:
    ...
    void createUI(void *parent) override {
        ...
        lv_obj_add_event_cb(mkdirBtn, [](lv_event_t *e) {
            FilesApp *app = (FilesApp *)lv_event_get_user_data(e);
        ...
        lv_obj_add_event_cb(m_pasteBtn, [](lv_event_t *e) {
            FilesApp *app = (FilesApp *)lv_event_get_user_data(e);
            app->pasteItem();
        }, LV_EVENT_CLICKED, this);
...
private:
    ...
    std::string toVfsPath(const std::string& lvPath) {
        if (lvPath.substr(0, 2) == "A:") return lvPath.substr(2);
        return lvPath;
    }
    ...
    void refreshList() {
        ...
        if (res != LV_FS_RES_OK && (m_currentPath == "A:/" || m_currentPath == "A:")) {
    ...
    void addListItem(const std::string &name, bool isDir) {
        ...
        lv_obj_add_event_cb(btn, [](lv_event_t *e) {
            ...
            if (code == LV_EVENT_CLICKED) {
                ...
                if (isDir) app->enterDir(name);
                else app->onFileClick(name);
            }
        }, LV_EVENT_ALL, this);
    }
    ...
    void handleMenuAction(const std::string &action, const std::string &name, bool isDir) {
        if (action == "Copy") {
            ...
            lv_msgbox_add_text(confirm, ("Are you sure you want to delete " + name + "?").c_str());
            lv_obj_t * btn_yes = lv_msgbox_add_footer_button(confirm, "Yes");
            lv_msgbox_add_close_button(confirm);

            struct ConfirmCtx { FilesApp* app; std::string name; bool isDir; lv_obj_t* mbox; };
    ...
    void showInputDialog(const char* title, const std::string& defaultVal, std::function<void(std::string)> cb) {
        ...
        struct InputCtx { std::function<void(std::string)> cb; lv_obj_t* ta; lv_obj_t* mbox; };
    ...
    int removeRecursive(const char *path) {
        ...
        while ((p = readdir(d))) {
            ...
            if (stat(subPath.c_str(), &st) == 0) {
                if (S_ISDIR(st.st_mode)) r = removeRecursive(subPath.c_str());
                else r = unlink(subPath.c_str());
            } else r = -1;
            if (r != 0) break;
        }
        closedir(d);
