...
public:
  ...
  void show() {
    if (m_container == nullptr) {
      ...
      lv_obj_bind_checked(m_btSwitch, &ConnectivityManager::getInstance().getBluetoothEnabledSubject());
      ...
      lv_label_set_text(m_statusLabel, BluetoothManager::getInstance().isEnabled() ? "Ready" : "Disabled");
  ...
  void startScan() {
    if (!BluetoothManager::getInstance().isEnabled()) return;
    lv_obj_clean(m_list);
