...
public:
  ...
  void createUI(void *parent) override {
    ...
    m_mainList = nullptr;
    m_wifiSettings = std::make_unique<Settings::WiFiSettings>(m_container, [this]() { showMainSettings(); });
    m_hotspotSettings = std::make_unique<Settings::HotspotSettings>(m_container, [this]() { showMainSettings(); });
    m_bluetoothSettings = std::make_unique<Settings::BluetoothSettings>(m_container, [this]() { showMainSettings(); });
    m_displaySettings = std::make_unique<Settings::DisplaySettings>(m_container, [this]() { showMainSettings(); });
    showMainSettings();
  }
  ...
  void onStop() override {
    m_container = nullptr;
    m_mainList = nullptr;
    if (m_wifiSettings) m_wifiSettings->destroy();
    if (m_hotspotSettings) m_hotspotSettings->destroy();
    if (m_bluetoothSettings) m_bluetoothSettings->destroy();
    if (m_displaySettings) m_displaySettings->destroy();
...
private:
  ...
  void showMainSettings() {
    ...
    if (m_mainList == nullptr) {
      ...
      lv_obj_t *wifiBtn = Settings::add_list_btn(m_mainList, LV_SYMBOL_WIFI, "Wi-Fi");
      lv_obj_add_event_cb(
          wifiBtn,
          [](lv_event_t *e) {
      ...
      lv_obj_t *hotspotBtn = Settings::add_list_btn(m_mainList, LV_SYMBOL_WIFI, "Hotspot"); // LV_SYMBOL_WIFI is used for hotspot too if no specific one
      lv_obj_add_event_cb(
          hotspotBtn,
          [](lv_event_t *e) {
            SettingsApp *app = (SettingsApp *)lv_event_get_user_data(e);
      ...
      lv_obj_t *btBtn = Settings::add_list_btn(m_mainList, LV_SYMBOL_BLUETOOTH, "Bluetooth");
      lv_obj_add_event_cb(
          btBtn,
          [](lv_event_t *e) {
            SettingsApp *app = (SettingsApp *)lv_event_get_user_data(e);
