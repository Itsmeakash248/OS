...
public:
  ...
  void show() {
    if (m_container == nullptr) {
      ...
      lv_obj_bind_checked(m_wifiSwitch, &ConnectivityManager::getInstance().getWiFiEnabledSubject());
      ...
      lv_obj_set_flex_align(statusCont, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
      ...
      if (m_timer) lv_timer_resume(m_timer);
      updateStatus();
      refreshScan();
    }
  }
  ...
  void refreshScan() {
    if (m_list == nullptr || m_isScanning) return;
    ...
    ConnectivityManager::getInstance().scanWiFi([this](std::vector<wifi_ap_record_t> networks) {
      GuiTask::lock();
      m_isScanning = false;
      if (m_list == nullptr) {
        GuiTask::unlock();
  ...
  const char* getSignalIcon(int8_t rssi) {
    ...
    if (rssi >= -70) return LV_SYMBOL_WIFI; // Could use more granular icons if theme supports
  ...
  void updateStatus() {
    ...
    WiFiStatus status = static_cast<WiFiStatus>(lv_subject_get_int(&ConnectivityManager::getInstance().getWiFiStatusSubject()));
    
    switch(status) {
      case WiFiStatus::DISABLED: lv_label_set_text(m_statusLabel, "Disabled"); break;
      case WiFiStatus::DISCONNECTED: lv_label_set_text(m_statusLabel, "Disconnected"); break;
      case WiFiStatus::SCANNING: break;
      case WiFiStatus::CONNECTING: lv_label_set_text(m_statusLabel, "Connecting..."); break;
      case WiFiStatus::CONNECTED: {
        wifi_ap_record_t ap_info;
        if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
          const char *auth_symbol = (ap_info.authmode == WIFI_AUTH_OPEN) ? LV_SYMBOL_EYE_OPEN : LV_SYMBOL_EYE_CLOSE;
      ...
      case WiFiStatus::AUTH_FAILED: lv_label_set_text(m_statusLabel, "Authentication Failed"); break;
      case WiFiStatus::NOT_FOUND: lv_label_set_text(m_statusLabel, "Network Not Found"); break;
    }
  }
  ...
  void showConnectScreen(const char *ssid) {
    ...
    lv_obj_set_style_bg_color(m_connectContainer, lv_palette_main(LV_PALETTE_GREY),
                              0);
    lv_obj_set_flex_flow(m_connectContainer, LV_FLEX_FLOW_COLUMN);
    lv_obj_set_flex_align(m_connectContainer, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                          LV_FLEX_ALIGN_CENTER);
    ...
    lv_obj_add_event_cb(
        ...
        [](lv_event_t *e) {
          ...
          ConnectivityManager::getInstance().connectWiFi(instance->m_connectSsid.c_str(), password);
          lv_obj_delete(instance->m_connectContainer);
          instance->m_connectContainer = nullptr;
          instance->m_passwordTa = nullptr;
          instance->updateStatus();
