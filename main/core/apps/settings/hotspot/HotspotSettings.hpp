...
private:
  void createMainPage() {
    ...
    lv_obj_bind_checked(m_hotspotSwitch, &ConnectivityManager::getInstance().getHotspotEnabledSubject());

    lv_obj_add_event_cb(m_hotspotSwitch, [](lv_event_t *e) {
    ...
    lv_obj_add_event_cb(confBtn, [](lv_event_t *e) {
      auto *instance = (HotspotSettings *)lv_event_get_user_data(e);
    ...
    lv_subject_add_observer_obj(&ConnectivityManager::getInstance().getHotspotUptimeSubject(),
      [](lv_observer_t *o, lv_subject_t *s) {
        ...
        int m = (sec % 3600) / 60;
        int r = sec % 60;
        if (h > 0) lv_label_set_text_fmt(label, "Uptime: %dh %dm %ds", h, m, r);
        else if (m > 0) lv_label_set_text_fmt(label, "Uptime: %dm %ds", m, r);
        else lv_label_set_text_fmt(label, "Uptime: %ds", r);
      }, uptimeLabel, nullptr);

    lv_obj_t *downSpeedLabel = lv_label_create(usageCont);
    lv_label_set_text(downSpeedLabel, "Download: 0 KB/s");
    lv_subject_add_observer_obj(&ConnectivityManager::getInstance().getHotspotDownloadSpeedSubject(),
      [](lv_observer_t *o, lv_subject_t *s) {
        lv_obj_t *label = lv_observer_get_target_obj(o);
        int32_t kb = lv_subject_get_int(s);
        if (kb > 1024) lv_label_set_text_fmt(label, "Download: %.2f MB/s", (float)kb / 1024.0f);
        else lv_label_set_text_fmt(label, "Download: %d KB/s", (int)kb);
      }, downSpeedLabel, nullptr);

    lv_obj_t *upSpeedLabel = lv_label_create(usageCont);
    lv_label_set_text(upSpeedLabel, "Upload: 0 KB/s");
    lv_subject_add_observer_obj(&ConnectivityManager::getInstance().getHotspotUploadSpeedSubject(),
      [](lv_observer_t *o, lv_subject_t *s) {
        lv_obj_t *label = lv_observer_get_target_obj(o);
        int32_t kb = lv_subject_get_int(s);
        if (kb > 1024) lv_label_set_text_fmt(label, "Upload: %.2f MB/s", (float)kb / 1024.0f);
        else lv_label_set_text_fmt(label, "Upload: %d KB/s", (int)kb);
      }, upSpeedLabel, nullptr);

    lv_obj_t *sentLabel = lv_label_create(usageCont);
    lv_label_set_text(sentLabel, "Sent: 0 KB");
    lv_subject_add_observer_obj(&ConnectivityManager::getInstance().getHotspotUsageSentSubject(),
      [](lv_observer_t *o, lv_subject_t *s) {
        lv_obj_t *label = lv_observer_get_target_obj(o);
        int32_t kb = lv_subject_get_int(s);
        if (kb > 1024) lv_label_set_text_fmt(label, "Sent: %.2f MB", (float)kb / 1024.0f);
        else lv_label_set_text_fmt(label, "Sent: %d KB", (int)kb);
      }, sentLabel, nullptr);

    lv_obj_t *recvLabel = lv_label_create(usageCont);
    lv_label_set_text(recvLabel, "Received: 0 KB");
    lv_subject_add_observer_obj(&ConnectivityManager::getInstance().getHotspotUsageReceivedSubject(),
      [](lv_observer_t *o, lv_subject_t *s) {
        lv_obj_t *label = lv_observer_get_target_obj(o);
        int32_t kb = lv_subject_get_int(s);
        if (kb > 1024) lv_label_set_text_fmt(label, "Received: %.2f MB", (float)kb / 1024.0f);
        else lv_label_set_text_fmt(label, "Received: %d KB", (int)kb);
      }, recvLabel, nullptr);
    ...
    lv_obj_add_event_cb(resetUsageBtn, [](lv_event_t *e) {
    ...
    lv_subject_add_observer_obj(&ConnectivityManager::getInstance().getHotspotClientsSubject(),
      [](lv_observer_t *o, lv_subject_t *s) {
        lv_obj_t *label = lv_observer_get_target_obj(o);
        lv_label_set_text_fmt(label, "Connected Clients (%d):", (int)lv_subject_get_int(s));
    ...
    if (m_refreshTimer == nullptr) {
      m_refreshTimer = lv_timer_create([](lv_timer_t * t) {
        auto * instance = (HotspotSettings *)lv_timer_get_user_data(t);
        if (!instance->m_clientsCont || lv_obj_has_flag(instance->m_mainPage, LV_OBJ_FLAG_HIDDEN)) return;

        lv_obj_clean(instance->m_clientsCont);
        auto clients = ConnectivityManager::getInstance().getHotspotClientsList();
        if (clients.empty()) {
          ...
          for (const auto& client : clients) {
            ...
            if (client.rssi < -80) rssi_str = "Poor";
            else if (client.rssi < -70) rssi_str = "Fair";
            else if (client.rssi < -60) rssi_str = "Good";

            uint32_t d = client.connected_duration;
            if (d > 3600) {
              lv_label_set_text_fmt(info_label, "IP: %s | %s | %dh %dm", 
                                    client.ip.empty() ? "Allocating..." : client.ip.c_str(), 
                                    rssi_str, (int)(d/3600), (int)((d%3600)/60));
            } else {
              lv_label_set_text_fmt(info_label, "IP: %s | %s | %dm %ds", 
                                    client.ip.empty() ? "Allocating..." : client.ip.c_str(), 
  ...
  void createConfigPage() {
    ...
    lv_obj_t *backBtn;
    lv_obj_t *header = create_header(m_configPage, "Configure Hotspot", &backBtn);
    ...
    lv_obj_add_event_cb(m_maxConnSlider, [](lv_event_t *e) {
    ...
    lv_obj_add_event_cb(m_txPowerSlider, [](lv_event_t *e) {
      lv_obj_t *slider = lv_event_get_target_obj(e);
      lv_obj_t *label = (lv_obj_t *)lv_event_get_user_data(e);
      lv_label_set_text_fmt(label, "%d", (int)lv_slider_get_value(slider));
  ...
  void applyHotspotSettings() {
    ...
    if (auth_idx == 0) auth = WIFI_AUTH_OPEN;
    else if (auth_idx == 1) auth = WIFI_AUTH_WPA2_PSK;
    else if (auth_idx == 2) auth = WIFI_AUTH_WPA3_PSK;
    else if (auth_idx == 3) auth = WIFI_AUTH_WPA2_WPA3_PSK;
    ...
    HotspotManager::getInstance().setAutoShutdownTimeout(lv_obj_has_state(m_autoShutdownSwitch, LV_STATE_CHECKED) ? 300 : 0);
    esp_err_t err = ConnectivityManager::getInstance().startHotspot(ssid, pass, channel, max_conn, hidden, auth, tx_power);
    if (err != ESP_OK) {
      lv_obj_remove_state(m_hotspotSwitch, LV_STATE_CHECKED);
    }
  }
